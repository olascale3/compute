...
## API Routes
- `GET /api/login` - Start Replit OIDC login (Google, GitHub, Apple, X, email)
- `GET /api/callback` - Replit OIDC callback
- `GET /api/logout` - Logout (clears session + Replit OIDC end session)
...
## Recent Changes
- 2026-02-21: Switched to Replit Auth (OIDC) as primary sign-in method. Users can sign in with Google, GitHub, Apple, X, or email through Replit's auth system. Email/password still available as secondary option.
